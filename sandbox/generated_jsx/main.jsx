...
<PartialsHello {...this.props}  />{!!this.props.people ? (  v = this.props.people,  toString.call(v) === "[object Object]" ? (v = [v]) : (null),  toString.call(v) === "[object Array]" ? (    v.map(function(el, ndx) {      return (    <div  key={ndx}>
      <p id={ '' + ((!!element_id) ? (element_id) : (''))} className={(" " + "something")+(!!b_show_class ? (" " + "something-else") : "")}>{!!(!!el && !!el.name) ? (el.name) : (!!(!!this.state && !!this.state.name) ? (this.state.name) : (!!(!!this.props && !!this.props.name) ? (this.props.name) : (null)))}</p>
    </div>
);    }.bind(this))  ) : (    <div  key={ndx}>
      <p id={ '' + ((!!element_id) ? (element_id) : (''))} className={(" " + "something")+(!!b_show_class ? (" " + "something-else") : "")}>{!!(!!this.state && !!this.state.name) ? (this.state.name) : (!!(!!this.props && !!this.props.name) ? (this.props.name) : (null))}</p>
    </div>
